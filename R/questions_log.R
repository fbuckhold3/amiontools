...
#'   or a future 5th "Other" choice once added to the dictionary). One row
#'   per logged entry — a resident can have zero, one, or more than one
#'   entry per date (e.g. both SLUH and VA logged the same day is possible,
#'   though unusual).
#' @export
pull_questions_log <- function(rdm_token, redcap_url) {
  ...
  if (nrow(raw) == 0) {
    return(data.frame(record_id = character(), Date = as.Date(character()),
                       q_conference_type = character(), stringsAsFactors = FALSE))
  }
  ...
  raw[!is.na(raw$Date), c("record_id", "Date", "q_conference_type")]
}
